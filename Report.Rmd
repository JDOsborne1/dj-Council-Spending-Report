...
extract <- links %>% 
    dataExtraction(control_links = T)
...
date_level_spending_data <- refined_extract %>% 
        ...
        mutate(Total.Spend.so.far = cumsum(GL.Code.Net.Amount)) %>% 
        group_by(Payment.date) %>% 
        filter(Total.Spend.so.far == max(Total.Spend.so.far)) %>% 
        ungroup()

dept_level_spending_data <- refined_extract %>% 
...
test <- customDataReader(URL = URL_parsing_failure_4)
```
